...
class _RegistrationUIState extends State<RegistrationUI> {
  ...
  registerUser() async {
    ...
    var request = http.MultipartRequest(
        'POST',
        Uri.parse(
            'https://developmentalphawizz.com/financego/app/v1/api/user_register'));
    ...
    if (response.statusCode == 200) {
      ...
      print('This is responseeeeeeeeeee${finalresponse}');

      if (jsonresponse['error'] == false) {
        var id = jsonresponse['data'][0]['id'];
        print('This is id____________${id.runtimeType}');
        SharedPreferences preferences = await SharedPreferences.getInstance();
        preferences.setString("userid", "${id}");
        print("thissssssssssssssssssssssss${id}");

        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("User Register Success")));
